...
from data import fetch_data, build_features
...
def run_pipeline(
    ticker: str, window_days: int = 365, n_lags: int = 10,
    n_optuna_trials: int = 20, val_ratio: float = 0.2,
    min_train: int = 60, refit_freq: int = 1, output_dir: str = "outputs"
) -> dict:
    ...
    if len(X) < min_train + 10:
        raise ValueError(f"Not enough data after feature engineering ({len(X)} rows). Increase window_days.")

    best_params, best_val_mae = bayesian_optimisation(X, y, n_trials=n_optuna_trials, val_ratio=val_ratio)
    results, metrics = run_backtest(X, y, best_params=best_params, min_train=min_train, refit_freq=refit_freq)
    ...
    report = performance_report(
        ...
        output_dir=output_dir, val_split_date=val_split_date
    )
    report["best_val_mae"] = round(best_val_mae, 6)
    report["best_params"] = {k: v for k, v in best_params.items() if k not in ("objective", "metric", "verbosity", "n_jobs", "random_state")}
...
if __name__ == "__main__":
    ...
    parser.add_argument("ticker", type=str, help="Contract ticker (e.g. KXBTCD-25MAR14-B94999)")
    ...
    parser.add_argument("--min-train", type=int, default=60, help="Min training samples for backtest")
    parser.add_argument("--refit-freq", type=int, default=1, help="Walk-forward refit frequency")
    parser.add_argument("--output-dir", type=str, default="outputs", help="Output directory")
    args = parser.parse_args()

    run_pipeline(
        ticker=args.ticker, window_days=args.days, n_lags=args.lags,
        n_optuna_trials=args.trials, val_ratio=args.val_ratio,
        min_train=args.min_train, refit_freq=args.refit_freq, output_dir=args.output_dir
    )