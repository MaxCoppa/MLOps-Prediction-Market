    ...
    for i in tqdm(range(min_train, len(dates)), desc="Backtesting", ncols=80):
        ...
        if should_refit:
            ...
            def calc_metrics(model, y_hat_te):
                ...
                r2_tr = float(r2_score(y_tr_arr[mask_tr], yhat_tr_a[mask_tr])) if mask_tr.sum() > 1 else np.nan
                acc_tr = float(accuracy_score(np.sign(y_tr_arr[mask_tr]), np.sign(yhat_tr_a[mask_tr]))) if mask_tr.sum() > 1 else np.nan
                
                r2_te = float(r2_score(y_true[mask_te], y_hat_te[mask_te])) if mask_te.sum() > 1 else np.nan
                acc_te = float(accuracy_score(np.sign(y_true[mask_te]), np.sign(y_hat_te[mask_te]))) if mask_te.sum() > 1 else np.nan
            ...
            metrics_opt.append({"date": test_date, "r2_train": r2_tr_o, "acc_train": acc_tr_o, "r2_test": r2_te_o, "acc_test": acc_te_o})
            metrics_def.append({"date": test_date, "r2_train": r2_tr_d, "acc_train": acc_tr_d, "r2_test": r2_te_d, "acc_test": acc_te_d})

    # Convert to DataFrames
    res_df_opt = pd.DataFrame(results_opt).set_index(["date", "ticker"]).sort_index() if results_opt else pd.DataFrame()
    met_df_opt = pd.DataFrame(metrics_opt).set_index("date") if metrics_opt else pd.DataFrame()
    
    res_df_def = pd.DataFrame(results_def).set_index(["date", "ticker"]).sort_index() if results_def else pd.DataFrame()
    met_df_def = pd.DataFrame(metrics_def).set_index("date") if metrics_def else pd.DataFrame()
...
def performance_report(
    ...
    if has_base:
        ...
        r2_d = float(r2_score(valid_d["y_true"], valid_d["y_hat"])) if len(valid_d) > 1 else np.nan
        acc_d = float(accuracy_score(np.sign(valid_d["y_true"]), np.sign(valid_d["y_hat"]))) if len(valid_d) > 1 else np.nan
    ...
    ax_pnl.plot(cum_pnl.index, cum_pnl.values, color="#1f77b4", linewidth=1.5, label="Optimized PnL (OOS)")
    if has_base:
    ...
    ax_pnl.set_title(f"Cumulative PnL (equal-weight, {report['n_tickers']} tickers) — {series_ticker}")
    ...
    if has_base:
        table_data = [
            ...
            ["Total PnL", str(report['total_pnl']), str(round(float(cum_pnl_d.iloc[-1]), 4))],
            ...
            ["R² OOS", str(report.get('global_r2_test', 'N/A')), str(round(r2_d, 4)) if not np.isnan(r2_d) else "N/A"],
            ["Acc OOS", str(report.get('global_acc_test', 'N/A')), str(round(acc_d, 3)) if not np.isnan(acc_d) else "N/A"]
        ]
        ...
        if metrics_default is not None and not metrics_default.empty:
            ...
            table_data.extend([
                ...
                ["Acc Train", str(report.get('mean_acc_train', 'N/A')), str(acc_tr_d)]
            ])
