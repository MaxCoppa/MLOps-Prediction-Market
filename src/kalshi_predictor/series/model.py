...
def bayesian_optimisation(
    ...
    log.info(
        f"Bayesian optimisation — train={len(X_tr)}, val={len(X_val)}, n_trials={n_trials}"
    )
    ...
    log.info("Best MSE (val): %.6f", study.best_value)
    clean_params = {
        k: v
        for k, v in best_params.items()
        if k not in ("objective", "metric", "verbosity", "n_jobs", "random_state")
    }
    log.info("Best params: %s", json.dumps(clean_params, indent=2))
