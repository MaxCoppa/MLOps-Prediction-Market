    ...
    for i in tqdm(range(min_train, len(days)), desc="Backtesting", ncols=80):
        ...
        if (i - min_train) % refit_freq == 0:
            ...
            if mask.sum() > 1:
                ...
                acc_tr = float(
                    accuracy_score(np.sign(y_tr_arr[mask]), np.sign(yhat_tr_arr[mask]))
                )
            else:
...
def performance_report(
    ...
    if not res_filt.empty and len(res_filt) > 1:
        report["global_r2_test"] = round(
            float(r2_score(res_filt["y_true"], res_filt["y_hat"])), 4
        )
        report["global_acc_test"] = round(
            float(accuracy_score(y_true_sgn, y_hat_sgn)), 3
        )
    ...
    ax_plot.plot(
        ...
        color="#1f77b4",
        linewidth=1.5,
        label="Walk-Forward PnL (OOS)",
    )
    if (
        val_split_date is not None
        and val_split_date >= cum_pnl.index[0]
        and val_split_date <= cum_pnl.index[-1]
    ):
        ax_plot.axvline(
            x=val_split_date,
            color="red",
